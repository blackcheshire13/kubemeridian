    ...
    get status() {
        ...
        if (this.data.metadata?.deletionTimestamp) {
            ...
            if (st.containerStatuses) {
                containerStatuses = st.containerStatuses.filter((item: any) => item.ready === false);
                containerStatuses.length > 0 ? (containerStatuses = true) : (containerStatuses = false);
            }
            if (st.initContainerStatuses) {
                initContainerStatuses = st.initContainerStatuses.filter((item: any) => item.ready === false);
    ...
    get message() {
        ...
        if (status === ERROR) {
            ...
            if (this.data.metadata?.deletionTimestamp) {
                ...
                if (data.status?.containerStatuses) {
                    let d = data.status.containerStatuses.filter((item: any) => item.ready === false)[0];
                    if (d && d.state?.waiting?.message && d.state.waiting.message.length > 0) {
